...
AS SELECT 
	*
	FROM 
		...
		JOIN statuses as st 
		ON inc.status_id = st.id
		WHERE rel.record_1_type_id = 1 AND rel.record_2_type_id = 2 
...
AS SELECT 
	...
	FROM 
		...
		WHERE req.status_id = 1) AS open_rec
	JOIN contacts
...
ON inc.priority_id = pr.id
WHERE inc.status_id NOT IN (4, 5) AND inc.assignee_id = @user_id
...
JOIN statuses AS st 
ON req.status_id = st.id
JOIN priorities AS pr 
ON req.priority_id = pr.id
WHERE req.status_id NOT IN (4, 5) AND req.assignee_id = @user_id;
...
JOIN statuses AS st 
ON inc.status_id = st.id
WHERE record_1_type_id = 4 AND record_2_type_id = 2;
...
FOR EACH ROW
BEGIN
	IF((NEW.resolution IS NULL OR NEW.resolution = '' OR NEW.closure_code_id IS NULL OR NEW.closure_code_id = '') AND NEW.status_id = 5) THEN
...
INSERT INTO knowledges (id, title, description, author_id, type_id) 
VALUES (21, '', NULL, 1, 5);
...
BEGIN
	UPDATE requests
	SET status_id = 2
	WHERE status_id = 3 AND updated_at < (NOW() - INTERVAL 7 DAY);
END//
DELIMITER ;
